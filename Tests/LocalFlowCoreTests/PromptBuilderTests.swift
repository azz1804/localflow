...
final class PromptBuilderTests: XCTestCase {
    ...
    func testPromptModePreservesCodingDetailsWithoutAnswering() {
        ...
        XCTAssertTrue(systemPrompt.contains("same person after one thoughtful edit"))
        XCTAssertTrue(systemPrompt.contains("first-person phrasing"))
        XCTAssertTrue(systemPrompt.contains("deliberate profanity"))
        XCTAssertTrue(systemPrompt.contains("every stated reason"))
        XCTAssertTrue(systemPrompt.contains("never sanitize the reaction"))
        XCTAssertTrue(systemPrompt.contains("Keep short and medium requests as natural prose"))
